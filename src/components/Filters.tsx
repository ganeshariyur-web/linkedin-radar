...
export function Filters({ tab, rows, filters, onChange }: { tab: Tab; rows: ScoredRow[]; filters: F; onChange: (f: F) => void }) {
  ...
  const active = filters.tier.length + filters.role.length + filters.companyType.length + filters.country.length + filters.sizeBand.length + filters.activity.length + filters.connectedFor.length + (filters.hasEmail !== "any" ? 1 : 0) + (filters.privateFamily !== "any" ? 1 : 0) + (filters.watchlist !== "any" ? 1 : 0) + (filters.enriched !== "any" ? 1 : 0) + (filters.text ? 1 : 0);
  return (
    <div className="space-y-3" data-testid="filters">
      <div className="flex items-center gap-3">
        ...
        {active > 0 && <button className="btn ghost" onClick={() => onChange({ ...filters, tier: [], role: [], companyType: [], country: [], sizeBand: [], activity: [], connectedFor: [], hasEmail: "any", privateFamily: "any", watchlist: "any", enriched: "any", text: "" })}>Clear {active}</button>}
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        {rows.some((r) => r.enrichment) && <Tri title="Enriched" value={filters.enriched} onChange={(v) => set("enriched", v)} />}
